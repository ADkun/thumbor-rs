...
lazy_static! {
    ...
    static ref WATERMARK: PhotonImage = {
        // 这里你需要把github项目下对应图片拷贝到你的根目录
        // 编译时 include_bytes! 宏会直接把文件读入编译后的二进制
...
impl SpecTransform<&Flipv> for Photon {
    fn transform(&mut self, _v: &Flipv) {
        transform::flipv(&mut self.0);
    }
}

impl SpecTransform<&Fliph> for Photon {
    fn transform(&mut self, _v: &Fliph) {
...
impl SpecTransform<&Resize> for Photon {
    fn transform(&mut self, v: &Resize) {
        let img = match resize::ResizeType::from_i32(v.rtype).unwrap() {
            ...
            resize::ResizeType::SeamCarve => transform::seam_carve(&mut self.0, v.width, v.height),
        };
...
// photon 库没有提供在内存中对图片转换格式的方法，只好施工实现
